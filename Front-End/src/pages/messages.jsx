      ...
      <h2 className='hidden md:block mt-3 text-center text-xl h-[45px] border-b border-white/20 font-bold'>Connect Via message</h2>
        <div className=" w-full h-[80vh] flex flex-col items-center ">
          ...
          <div className="w-full flex flex-col items-center mt-8  h-[85%] gap-5">
            <div onClick={()=>setMessageType('normalMessage')} className={`${messageType === 'normalMessage' ? 'h-[400px]' : 'h-[40px]'} w-[70%] bg-white/95 flex flex-col items-center gap-4  overflow-y-hidden pt-1.5 transition-all rounded-3xl px-4 py-3 border`}>
            <p className='text-black'>Send a message</p>
              
                <input type="text" className="border-b border-black p-1 placeholder:text-black text-black outline-0" placeholder='Your name' />
                <input type="email" className="border-b p-1 border-black placeholder:text-black text-black outline-0" placeholder='Email address' />
              
              <textarea name="" id="" placeholder='write message here' className='resize-none outline-0 p-3 bg-black/20 rounded-2xl w-[90%] h-[160px] placeholder:text-sm text-black placeholder:text-black text-sm  mt-5 no-scrollbar'></textarea>
              <button className="bg-green-500 text-white px-4 py-1.5 rounded-2xl text-sm ">Send message</button>
            </div>
            <div onClick={()=>setMessageType('anonymous')} className={`${messageType === 'anonymous' ? 'h-[300px]' : 'h-[40px]'} w-[70%] flex flex-col gap-5 overflow-y-hidden items-center pt-1.5 transition-all rounded-3xl px-4  border`}>
              <p>Send an anonymous message</p>
              <textarea name="" id="" placeholder='write message here' className='resize-none outline-0 p-3 bg-white/10 rounded-2xl w-[90%] h-[160px] placeholder:text-sm text-sm  mt-5 no-scrollbar'></textarea>
